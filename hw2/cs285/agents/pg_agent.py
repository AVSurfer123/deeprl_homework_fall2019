...
class PGAgent(BaseAgent):
    ...
    def _discounted_return(self, rewards):
        ...
        discounted_rewards = [discounts[i] * rewards[i] for i in indices]
        ...
        list_of_discounted_returns = [sum_of_discounted_rewards] * (T-1)
    ...
    def _discounted_cumsum(self, rewards):
        ...
        for start_time_index in range(len(rewards)): 
            ...
            indices = list(range(start_time_index, T))

            # 2) create a list where the entry at each index (t') is gamma^(t'-t)
            discounts = [self.gamma ** (i - start_time_index) for i in indices]
            ...
            discounted_rtg = [d * r for d, r in zip(discounts, rewards[start_time_index:])]
